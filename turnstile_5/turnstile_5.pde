...
void setup()
{
  ...
  size(round((float)spanLng * 2000), round((float)spanLat * 2000), P2D);
...
//  blend(ADD);  
  ...
  for (Object stationID : parser.stations.keySet()) {
    ...
    //fill(plotBrightness);
    //ellipse(x, y, 10, 10);        
    drawGradient(100, x, y, scalarVal);
  }
...
void drawGradient(float dim, float x, float y, float depth) 
{
  int radius = floor(dim/2);
  float a = 1.0/float(radius); //random(0, 360.0f);
  for (int r = radius; r > 0; --r) {
    //fill(h, 90, 90);
    fill(255,255,255,a*255*depth);
    ellipse(x, y, r, r);
    //h = (h + 1) % 360;
  }
}
